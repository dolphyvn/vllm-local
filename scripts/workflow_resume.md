# Trading Data Processing Pipeline - Python Only

## Project Overview
Convert MT5/Sierra Chart CSV exports into optimized vector database format for RAG-based trading analysis.
...
### 1. Project Structure
```bash
mkdir -p ~/trading-ai-system/{data/raw,src}
...
## Data Format Expected from MT5/Sierra Chart

### CSV Format (from your exports):
```csv
timestamp,open,high,low,close,tick_volume,spread
2025-10-30 13:46:00,3981.55,3983.35,3981.55,3982.79,438,2
2025-10-30 13:47:00,3982.65,3983.28,3980.30,3981.66,435,2
2025-10-30 13:48:00,3981.62,3982.02,3979.68,3981.52,411,2
...
```

**Note**: Column names may vary. Script handles: `timestamp/time/date`, `tick_volume/volume/vol`
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("Example: python data_processor.py ../data/raw/XAUUSD_M15.csv")
        sys.exit(1)
...
```bash
# Process single file
python src/data_processor.py data/raw/XAUUSD_M15.csv

# Output will be saved as: XAUUSD_M15_processed.json
```

### Integration with Your RAG System
```python
from src.data_processor import TradingDataProcessor
from your_rag_system import add_documents_to_vectordb

# Process data
processor = TradingDataProcessor('data/raw/XAUUSD_M15.csv')
results = processor.process_all()
...
- **Narrative format**: Natural language LLM can understand
- **Rich context**: Includes all technical conditions
- **Outcome tracking**: Shows what happened after pattern
- **Metadata**: Enables filtering by date, outcome, pattern type
- **Lessons**: Includes analysis of why it worked/failed
...
1. **Process all your CSV files** from MT5/Sierra
2. **Review output JSON** to verify format
3. **Integrate with your RAG system** using the documents
4. **Query with LLM** using narrative format

The documents are designed to give your LLM rich context about historical patterns and their outcomes!
